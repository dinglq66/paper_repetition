...
class WindowAttention(nn.Module):
    """
    对应论文中提出的W-MSA(Windows Multi-head Self-Attention)模块，这样做是为了减少计算量，即计算自注意力时只对窗口内的像素进行计算
    模块实现中包含了相对位置偏置
    Args:
        dim (int): 输入通道的数量
        window_size (tuple[int]): 窗口的高度和宽度
        num_heads (int): 注意力头的数量
        qkv_bias (bool, optional): 如果设置为True，q，k，v的生成过程中引入了可学习的偏置参数
        attn_drop (float, optional): 注意力权重的随机丢弃概率，默认为0.
        proj_drop (float, optianal): 模块输出部分的随机丢弃概率，默认为0.
    """
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        q, k, v = qkv.unbind(0)  # unbind函数按照指定的维度将张量划分为元组

        # transpose: --> [batch_size*num_windows, num_heads, embed_dim_per_head, Mh*Mw]
        # @: multiply --> [batch_size*num_windows, num_heads, Mh*Mw, Mh*Mw]
        attn = (q @ k.transpose(-2, -1)) * self.scale

        # relative_position_bias_table[...].view(...): [Mh*Mw*Mh*Mw, nH] --> [Mh*Mw, Mh*Mw, nH], nH表示num_heads
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1
        )
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # [nH, Mh*Mw, Mh*Mw]
        attn = attn + relative_position_bias.unsqueeze(0)  # unsqueeze()操作后将张量形状变为[1, nH, Mh*Mw, Mh*Mw]
        # 处理后的attn形状为: [batch_size*num_windows, num_heads, Mh*Mw, Mh*Mw]
...
class BasicLayer(nn.Module):
    ...
    def create_mask(self, x, H, W):
        # 计算SW-MSA中需要使用的attention mask
        # 要保证Hp和Wp是window_size的整数倍
        Hp = int(np.ceil(H / self.window_size)) * self.window_size
        Wp = int(np.ceil(W / self.window_size)) * self.window_size
        # 拥有和特征图一样的通道排列顺序，方便后续窗口划分
